...
function Allsnaps() {
  ...
  const [loading, setloading]=useState(false);
    ...
    async function getData()
    {
                ...
                //  if(getResult.allUsersChatFiles &&( getResult.allUsersChatFiles.length===1 ))
                //  {
                //  }
 ...
 axios.post('https://www2.executesimple.com/updateUserDownloads',formData, { headers })
   ...
   <div 
       className={!imgfullscreen?'viewbox':'showfullscreen'}>             
         {imgfullscreen && (
             <img src={viewimgsrc}
                  alt='profile images'
                  style={{ height:'100%', width:'90%',marginTop:'10px',
                           objectFit:'contain'}}/>
               )}           
         {!imgfullscreen && (
             <img src={viewimgsrc}
  ...
  <div>
   ...
   <div className='viewbox_social_icons'>
         ...
         <span className='social_icon'
             ...
             <ThumbUpAltIcon onClick={()=>addToliked(Data[imageindex].fileName)}/> 
             {likescount}
             <span className="tooltiptext1">click to Like</span>
         </span>
        

         <span className='social_icon' style={{color: isFav? 'green':'rgb(70, 69, 69)'}}>
             <FavoriteBorderIcon onClick={()=>addTofavourites(Data[imageindex].fileName)}/> 
             {favcount}
             <span className="tooltiptext1">Add to favourites</span>
         </span>

         <span className='social_icon'>
             <MmsIcon onClick={()=>commentSnaps(Data[imageindex].fileName)}/>
             {commentscount}
  ...
  {gotosearch && 
          ...
          {loading &&
            ...
            {!loading && Object.keys(Data).map((key, index) => {
         
         return(
          <div className={` ${shareimage.includes(Data[key].fileName)?'image-cardselected':'image-card'}`}
               key={Data[key].id}>
              <img className='image'
                   src={baseUrl+Data[key].fileName}
                    alt={Data[key].caption}
                   style={{width:'100%', height:'70%'}}/>
              { shareimage.includes(Data[key].fileName) && 
                      
                      <div style={{position:'absolute',
                                   backgroundColor:'rgb(2, 210, 16)',
                                   color:'white',
                                   cursor:'pointer'}}
                           onClick={()=>removefromsharelist(Data[key].fileName)}> 
                         <CheckBoxIcon/>
                      </div>
                    }
              {(Data[key].type!=='single')&&
                ...
                <div className='image__overlay--icon'> 
                   <FavoriteBorderIcon onClick={()=>addTofavourites1(Data[key])}/>
                </div>
                </div>
                */}
               {/* <p className="image-info"
                      style={{color:'grey', fontSize:'16px'}}>
                        {Data[key].caption===''? 'no data' : Data[key].caption}     
                 </p> */}
              <p className="image-info"
                 style={{color:'grey', fontSize:'16px',overflow:'hidden'}}>
                    {Data[key].category===''? 'no data' : Data[key].category}     
              </p>
              <div className='viewbutton'
                   style={{position:'static',padding:'6px 0', marginTop:'10px'}}>
                         ...
                         <span></span>
                         <button className='viewbuttonicons'>
                            <ShareIcon onClick={()=>imageclicked(index,Data)}/>
                         </button>                   
                       </div>
