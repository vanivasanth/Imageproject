...
import Remlikemodal from './Remlikemodal';
...
//import Multisharecart from './Multisharecart';
...
const Likes = () =>
{
   ...
   //const[imagelist, setimagelist]=useState({});
    ...
    async function getData() {
        ...
        // axios.get('https://www2.executesimple.com/getUserFiles', { headers })
        //     .then(response => {
        ...
        const api=`https://www2.executesimple.com/getUserFiles?search=likes`;
        const result=await fetch(api,{headers},{signal:controller.signal});
        console.log(result.result_code);
        const getResult= await result.json();      
        
        if(getResult.userLikeFiles &&( getResult.userLikeFiles.length>0 ))
        {
          setfileData(getResult.userLikeFiles);
    ...
    async function getsinglefileData(imagefilename)
    {
                   ...
                   const getResult= await result.json();
                   let propchk=getResult.chatFileDetails.hasOwnProperty('multiChatFiles');
                   console.log(propchk);
 ...
 <MultiImagetile   closemultimodel={closehandler}
     imagearray={multiimagelist}/>
          ...
          {gotosearch && 
              ...
              {/* {viewshare &&
                 <Multisharecart setCartOpen={setviewshare}/>} */}
              {delmodalOpen && 
                 <Remlikemodal setdelOpenModal={setdelModalOpen} 
                 ...
                 return(
                  <div className={` ${shareimage.includes(fileData[key].fileName)?'image-cardselected':'image-card'}`} 
                      ...
                      <CheckBoxIcon/>
                        ...
                        {/* <div className='image__overlay--icon'> 
                           <FavoriteBorderIcon onClick={()=>addTofavourites1(fileData[key])}/>
                        </div> 
                        <div className='image__overlay--icon'  onClick={()=>deleteimage(fileData[key].fileName)}> 
                           <DeleteOutlineIcon/>
                        </div>
                        </div>            */}
                      ...
                      <div className='viewbutton'
                           style={{position:'static', padding:'5px 0'}}>
